<?php require_once __DIR__ . '/layout.php'; start_layout('Dashboard'); ?>

<div id="alert-box"></div>

<div class="grid-4 mb-4" id="stats-grid">
  <div class="card"><div class="stat-value" id="stat-trips">—</div><div class="stat-label">My Trips</div></div>
  <div class="card"><div class="stat-value" id="stat-activities">—</div><div class="stat-label">Activities</div></div>
  <div class="card"><div class="stat-value" id="stat-expenses">—</div><div class="stat-label">Total Spent</div></div>
  <div class="card"><div class="stat-value" id="stat-polls">—</div><div class="stat-label">Open Polls</div></div>
</div>
...
<div class="grid-2">
  <div class="card">
    ...
    <div id="dash-trips"><div class="empty-state"><div class="icon">🗺</div>Loading…</div></div>
  </div>
  ...
  <div class="card">
    ...
    <div id="dash-activities"><div class="empty-state"><div class="icon">📅</div>Loading…</div></div>
  </div>
</div>

<script>
(async () => {
  const tripsRes = await API.get('trips', { action: 'list' });
  ...
  if (activeTrip) {
    const itRes = await API.get('itinerary', { action: 'list', trip_id: activeTrip.id });
    ...
    // Expenses stat
    const finRes = await API.get('financial', { action: 'list', trip_id: activeTrip.id });
    if (finRes.success) {
    ...
    // Polls stat
    const pollRes = await API.get('social', { action: 'polls', trip_id: activeTrip.id });
    if (pollRes.success) {
