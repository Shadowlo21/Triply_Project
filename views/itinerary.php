<?php require_once __DIR__ . '/layout.php'; start_layout('Itinerary'); ?>
...
<div class="modal-overlay hidden" id="modal-add-activity">
  <div class="modal">
    <div class="modal-header"><h3>Add Activity</h3><button class="modal-close" onclick="closeModal('modal-add-activity')">×</button></div>
      ...
      <form id="form-add-activity">
        ...
        <div class="form-group">
          ...
          <select name="transport_mode" class="form-control">
            ...
            <option value="train">Train</option><option value="flight">Flight</option><option value="walk">Walk</option>
          </select>
        </div>
...
<div class="modal-overlay hidden" id="modal-versions">
  <div class="modal">
    <div class="modal-header"><h3>Itinerary History</h3><button class="modal-close" onclick="closeModal('modal-versions')">×</button></div>
    <div class="modal-body"><div id="versions-list">Loading…</div></div>
  </div>
</div>

<div class="modal-overlay hidden" id="modal-comment">
  <div class="modal">
    <div class="modal-header"><h3>Comments</h3><button class="modal-close" onclick="closeModal('modal-comment')">×</button></div>
    <div class="modal-body">
...
async function loadTrips() {
  ...
  if (sel.options.length > 1) { sel.selectedIndex = 1; loadItinerary(); }
}

async function loadItinerary() {
  const tripId = document.getElementById('trip-select').value;
  if (!tripId) return;
  const res = await API.get('itinerary', { action: 'list', trip_id: tripId });
  ...
  const statusBadge = { confirmed: 'badge-green', draft: 'badge-yellow', cancelled: 'badge-gray' };
...
async function checkConflicts() {
  const tripId = document.getElementById('trip-select').value;
  if (!tripId) return;
  const res = await API.get('itinerary', { action: 'conflicts', trip_id: tripId });
...
document.getElementById('form-add-activity').addEventListener('submit', async e => {
  e.preventDefault();
  const tripId = document.getElementById('trip-select').value;
  if (!tripId) { showAlert('#add-alert', 'Select a trip first.'); return; }
  ...
  const res = await API.post('itinerary', {
    ...
    datetime: fd.get('datetime'), duration_min: fd.get('duration_min'),
    lat: fd.get('lat'), lng: fd.get('lng'),
    transport_mode: fd.get('transport_mode'),
  });
  setLoading(btn, false);
  if (res.success) { closeModal('modal-add-activity'); e.target.reset(); loadItinerary(); }
...
async function loadVersions() {
  const tripId = document.getElementById('trip-select').value;
  if (!tripId) return;
  const res = await API.get('itinerary', { action: 'versions', trip_id: tripId });
  const el = document.getElementById('versions-list');
  if (!res.success || !res.data.length) { el.innerHTML = '<p class="text-muted">No history yet.</p>'; return; }
...
async function rollback(versionId) {
  ...
  const res = await API.post('itinerary', { action: 'rollback', version_id: versionId });
  showAlert('#alert-box', res.message, res.success ? 'success' : 'error');
  if (res.success) { closeModal('modal-versions'); loadItinerary(); }
}

async function rsvp(activityId, status) {
  const res = await API.post('itinerary', { action: 'rsvp', activity_id: activityId, status });
  showAlert('#alert-box', res.message, res.success ? 'success' : 'error');
}
...
async function openComments(activityId) {
  ...
  const res = await API.get('itinerary', { action: 'comments', activity_id: activityId });
  const el = document.getElementById('comments-list');
  if (!res.success || !res.data.length) { el.innerHTML = '<p class="text-muted">No comments yet.</p>'; return; }
...
async function postComment() {
  ...
  const res = await API.post('itinerary', { action: 'comment', activity_id: currentActivityId, content });
  if (res.success) { document.getElementById('comment-text').value = ''; openComments(currentActivityId); }
}
